...
@Autonomous(name="Color Test Only", group="Autonomous")  // @Autonomous(...) is the other common choice
@Disabled
public class colorTestOnly extends LinearOpMode {
...
//****  STEP 1 ************ Drive straight, shoot, rotate to line
            ...
            robot.autoDriveForward(2300, 1); //was 2000
            // Shoot 2 particles
...
//**** STEP 2  ************Rotate and drive toward wall
           ...
           robot.rotateClockwiseEncoder(52,1);  //was 55
            ...
            robot.allWheelsStop();
            //    sleep(5000);
           robot.rotateClockwiseEncoder(32,1);
            robot.allWheelsStop();
            //
...
//            sleep(10); //was 100
            ...
            while ((realDist > 22) && opModeIsActive()) {
                realDist = robot.rangeSensor.getDistance(DistanceUnit.CM);
                robot.moveForward(.5);
...
// **** STEP 3  Find White Line
            ...
            telemetry.update();
            robot.strafeLeft(.3); // CAUTION WILL GO FOREVER
...
// **** STEP 4    Sense Colors and go and push 1st Beacon!!  *****************************
          ...
          /*  check = 1;
                while ((check == 1) && opModeIsActive())
                {
                    ...
                    */
                    /*
                    else if ((sensor2 == 0) || (sensor1 == 0) && opModeIsActive()) {
                        robot.autoDriveForward(50, 0.2);
                        check = 3;
                        break;
                        //#4
                    }
...
/*
                        ...
                        beaconDistStart = -1;
                        realDist = robot.rangeSensor.getDistance(DistanceUnit.CM);
                        while (realDist > beaconDistStart) {
                            realDist = robot.rangeSensor.getDistance(DistanceUnit.CM);
                ...
                while ((check == 2) && opModeIsActive())
                {
                    //#5
                        ...
                        telemetry.addData("Sensor 2", "%.2f", sensor2);
                        telemetry.update();
                        // if ((((sensor1 > blueLowRange && sensor1 < blueHighRange) && (sensor2 > blueLowRange && sensor2 < blueHighRange))) && opModeIsActive()) {
                        //   check = 3;
                        // robot.autoDriveBackward(900, .4);
                        //check2 = 2;
                    telemetry.addLine("#5");
                    telemetry.update();
                        // }
                        if ((sensor1 > blueLowRange && sensor1 < blueHighRange) && (sensor2 > blueLowRange && sensor2 < blueHighRange))
                        {
                            telemetry.addLine("in second loop");
                            check = 3;
                            robot.autoDriveBackward(900, .4);
...
// **** Step 5 Now go to other Line beacon
          //  robot.autoDriveBackward(500, .5);
/*            while ((realDist < 14) && opModeIsActive()) {
                realDist = robot.rangeSensor.getDistance(DistanceUnit.CM);
                robot.moveBackward(.6);
                robot.waitForTick(10);
            }
  */              //robot.rotateToDirection(telemetry, 90, 30, .8, .05);// correct if not at Rotate back to 90
    /*        robot.autoDriveBackward(800, .5);
            telemetry.addLine("#9");
            telemetry.update();
                robot.allWheelsStop();
                robot.autoStrafeLeft(4000, 1); //was 4000
                robot.allWheelsStop();
                //  robot.autoDriveForward(3000,.9);// Added
                // Might want to go half way and then redo angle if needed
                // robot.rotateToDirection(telemetry, 90,30,.2,.05);// correct if not at Rotate back to 90
             //   robot.allWheelsStop();
                /*robot.rotateToDirection(telemetry, 90, 30, .8, .05);// correct if not at Rotate back to 90
                while ((realDist > 19) && opModeIsActive()) {
                    realDist = robot.rangeSensor.getDistance(DistanceUnit.CM);
                    robot.moveForward(.4);
                    robot.waitForTick(10);
                }
        ...
        /*
                ...
                robot.strafeLeft(.3);//  CAUTION  will keep going forever?
                telemetry.addData("Line Tracker... ", "??");
                telemetry.update();
                ...
                robot.allWheelsStop();
                robot.rotateToDirection(telemetry, 90, 30, .2, .1);// correct if not at Rotate back to 90
            //robot.rotateToDirectionCloseEnough(telemetry, 90,2);
...
/*
       ...
       //     }
        ...
        //    robot.autoDriveBackward(500,1);
          // robot.rotateClockwiseEncoder(92,1);
            //robot.autoDriveForward(6500,1);
