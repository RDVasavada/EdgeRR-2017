...
public class EdgeBot {
    ...
    public void init(HardwareMap map) {
        ...
        frontLeftMotor = hMap.dcMotor.get("flm");
        frontRightMotor = hMap.dcMotor.get("frm");
        rearLeftMotor = hMap.dcMotor.get("rlm");
        rearRightMotor = hMap.dcMotor.get("rrm");

        setDriveMotorsRunUsingEncoders();

        // Initialize the lift motor
        liftMotor = hMap.dcMotor.get("lm");

        // Initialize the crane extension motor
        craneExtensionMotor = hMap.dcMotor.get("cem");

        // Initialize the jewel arm servos
        jewelLiftServo = hMap.servo.get("jls");
        jewelFlipServo = hMap.servo.get("jfs");

        resetJewelServos();

        // Initialize the clamp servos
        clampServoLeft = hMap.servo.get("csl");
        clampServoRight = hMap.servo.get("csr");

        openClampServos(1);

        // Initialize the crane servos
        craneRotateServo = hMap.crservo.get("crs");
        clawWristServo = hMap.servo.get("cws");
        clawPinchServo = hMap.servo.get("cps");
        ...
        leftColorSensor = hMap.colorSensor.get("lcs");
        rightColorSensor = hMap.colorSensor.get("rcs");
